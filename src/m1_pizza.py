...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    expected = [rg.Point(180, 100), rg.Point(120, 100)]
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(points)):
        smallcircles = rg.Circle(points[k], 10)
        smallcircles.fill_color = color
        smallcircles.attach_to(window)
        point = points[k]
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    title = 'PIZZA tests 4: a large number of thin black lines on yellow circle.'
    window = rg.RoseWindow(500, 500, title)
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(300, 300), 125)
...
def run_test_fancy_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'FANCY POLYGON test 4: 20 lime green lines on blue circle, hops = 2.'
    window = rg.RoseWindow(500, 500, title)
    circle = rg.Circle(rg.Point(250, 250), 150)
    circle.fill_color = 'orange'
    fancy_polygon(window, circle, 20, 9, 'red', 3)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(len(points)):
        if k < len(points) - hops_to_next_point:
            ...
            line.arrow = 'last'
            line.attach_to(window)
        elif k + hops_to_next_point >= len(points):
